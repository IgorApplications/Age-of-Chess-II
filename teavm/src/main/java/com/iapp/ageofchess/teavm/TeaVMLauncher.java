...
public class TeaVMLauncher {
    ...
    public static void main(String[] args) {
        var config = new TeaApplicationConfiguration("canvas");
        // change these to both 0 to use all available space, or both -1 for the canvas size.
        config.width = 0;
        config.height = 0;
        new TeaApplication(new ChessApplication(new RdTeaVmLauncher(), ServerMode.SERVER, ApplicationMode.RELEASE, LocalFeatures.USER), config);
    }
}
