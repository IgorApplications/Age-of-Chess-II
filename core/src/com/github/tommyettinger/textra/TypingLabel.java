...
public class TypingLabel extends TextraLabel {
    ...
    public TypingLabel(String text, Skin skin, String styleName, Font replacementFont, boolean markup) {
        this(text, skin.get(styleName, Label.LabelStyle.class), replacementFont, markup);
    }

    public TypingLabel(String text, Label.LabelStyle style, Font replacementFont, boolean markup) {
        super(text, style, replacementFont, markup);
        workingLayout.font(super.font);
    ...
    protected void setText(String newText, boolean modifyOriginalText) {
        if (modifyOriginalText && markup) newText = Parser.preprocess(newText);
...
//        layout.setTargetWidth(actualWidth);
    ...
    public void parseTokens() {
        if (markup) {
            String text = "{NORMAL}" + getDefaultToken();
            this.setText(Parser.preprocess(text + originalText), false, false);
            Parser.parseTokens(this);
        } else {
            this.setText(originalText.toString(), false, false);
        }
        parsed = true;
