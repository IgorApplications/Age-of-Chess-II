 ...
 * {@link RdSelectBox.RdSelectBoxStyle#background}.
 * @author mzechner
 * @author Nathan Sweet
 * @author Igor Ivanov
 * */
public class RdSelectBox<T> extends Widget implements Disableable {
    static final Vector2 temp = new Vector2();

    RdSelectBoxStyle style;
    ...
    public RdSelectBox(Skin skin) {
        this(skin.get(RdSelectBoxStyle.class));
    }

    public RdSelectBox(Skin skin, String styleName) {
        this(skin.get(styleName, RdSelectBoxStyle.class));
    }

    public RdSelectBox(RdSelectBoxStyle style) {
        setStyle(style);
        ...
        scrollPane = new RdSelectBox.SelectBoxScrollPane<>(this);
    ...
    public RdSelectBox(String styleName) {
    ...
    protected RdSelectBox.SelectBoxScrollPane<T> newScrollPane () {
        return new RdSelectBox.SelectBoxScrollPane<>(this);
    }
    ...
    public void setStyle (RdSelectBox.RdSelectBoxStyle style) {
    ...
    /** Returns the select box's style. Modifying the returned style may not have an effect until {@link #setStyle(RdSelectBoxStyle)}
     * is called. */
    public RdSelectBoxStyle getStyle () {
        return style;
    }
    ...
    public void layout () {
        ...
        if (title == null) {
            title = new RdLabel(" ",
                    new RdLabel.RdLabelStyle(style.font, style.fontColor));
        }
        ...
        if (bg != null) {
            prefHeight = Math.max(bg.getTopHeight() + bg.getBottomHeight() + title.getPrefHeight(), bg.getMinHeight());
        } else {
            prefHeight = title.getHeight();
        }
        ...
        if (selectedPrefWidth) {
            ...
            if (selected != null) {
                prefWidth += title.getPrefWidth();
            }
        } else {
            float maxItemWidth = 0;
            for (int i = 0; i < items.size; i++) {
                maxItemWidth = Math.max(getList().labels[i].getPrefWidth(), maxItemWidth);
            }

            prefWidth = maxItemWidth;
            if (bg != null) prefWidth = Math.max(prefWidth + bg.getLeftWidth() + bg.getRightWidth(), bg.getMinWidth());

            RdList.RdListStyle listStyle = style.listStyle;
    ...
    protected @Null Drawable getImageDrawable () {
        if (isDisabled() && style.backgroundDisabled != null) return style.imageDisable;
        if (scrollPane.hasParent() && style.backgroundOpen != null) return style.imageOpen;
        if (isOver() && style.backgroundOver != null) return style.imageOver;
        return style.image;
    }
    ...
    public void draw (Batch batch, float parentAlpha) {
        ...
        if (background != null) {
            ...
            y += (int) (height / 2 + background.getBottomHeight() + title.getHeight() / 2);
        } else {
            y += (int) (height / 2 + title.getHeight() / 2);
        }

        title.setEllipsis("...");
        title.setWidth(100);
        title.setColor(fontColor.r, fontColor.g, fontColor.b, fontColor.a * parentAlpha);
        title.setPosition(x, y);
        title.act(Gdx.graphics.getDeltaTime());
        title.draw(batch, getColor().a);

        drawImage(batch, getX() + width - height, getY() + height / 4, height);
    }
    ...
    public float getMaxSelectedPrefWidth () {
        ...
        float width = 0;
        for (int i = 0; i < items.size; i++) {
            width = Math.max(getList().labels[i].getWidth(), width);
        }
    ...
    protected void onShow (Actor scrollPane, boolean below) {
        ...
        scrollPane.addAction(sequence(fadeIn(0.3f, Interpolation.fade)));
    }
     ...
     * @author Nathan Sweet */
    static public class SelectBoxScrollPane<T> extends RdScrollPane {
        final RdSelectBox<T> selectBox;
        int maxListCount;
        ...
        public SelectBoxScrollPane (RdSelectBox<T> selectBox) {
         ...
         * {@link List#toString(Object)} to {@link RdSelectBox#toString(Object)}. */
        protected RdList<T> newList () {
            return new RdList<T>(selectBox.style.listStyle) {
        ...
        public RdSelectBox<T> getSelectBox () {
            return selectBox;
        }
    }

    /** The style for a select box, see {@link RdSelectBox}.
     * @author mzechner
     * @author Nathan Sweet */
    static public class RdSelectBoxStyle {
        public Font font;
        public Color fontColor = new Color(1, 1, 1, 1);
        public @Null Color overFontColor, disabledFontColor;
        public @Null Drawable background;
        public RdScrollPane.RdScrollPaneStyle scrollStyle;
        public RdList.RdListStyle listStyle;
        public @Null Drawable backgroundOver, backgroundOpen, backgroundDisabled;
        public @Null Drawable image, imageOver, imageOpen, imageDisable;
        public float scale = 1;

        public RdSelectBoxStyle() {}

        public RdSelectBoxStyle(Font font, Color fontColor, @Null Drawable background, RdScrollPane.RdScrollPaneStyle scrollStyle,
                                RdList.RdListStyle listStyle) {
            this.font = font;
        ...
        public RdSelectBoxStyle(RdSelectBoxStyle style) {
            font = style.font;
            ...
            scrollStyle = new RdScrollPane.RdScrollPaneStyle(style.scrollStyle);
            listStyle = new RdList.RdListStyle(style.listStyle);
