...
public class MultiplayerBoardView extends Image {
    ...
    public MultiplayerBoardView(MultiplayerGameController controller, Stage stage) {
    ...
    protected void sizeChanged() {
        ...
        var info = controller.getLocalMatch().getMatchData();
        double coefficientW = getWidth() / info.getWidth(),
                coefficientH = getHeight() / info.getHeight();

        padLeft = info.getPadLeft() * coefficientW;
        double padRight = info.getPadRight() * coefficientW;
        padBottom = info.getPadBottom() * coefficientH;
        double padTop = info.getPadTop() * coefficientH;
        ...
        if (Gdx.input.justTouched()) {
            ...
            for (var moveView : moveViews) {
                if (moveView.sprite.getBoundingRectangle().contains(spriteTouchPoint.x, spriteTouchPoint.y)) {
                    ...
                    controller.makeMove(moveView.move, null, true);
                }
            }
            ...
            if (hintView != null && hintView.sprite.getBoundingRectangle()
                ...
                hintView = null;
                controller.makeMove(move, null, true);
            }
        }
    }
