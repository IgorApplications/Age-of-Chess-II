 ...
 * <p>
 * The preferred height of a text field is the height of the {@link RdTextFieldStyle#font} and {@link RdTextFieldStyle#background}.
 ...
 *
 * */
public class RdTextField extends Widget implements Disableable {
    ...
    public RdLabel label, messageLabel;
    protected final FloatArray glyphPositions = new FloatArray();

    RdTextFieldStyle style;
    ...
    protected float fontOffset, textHeight, textOffset, minTextHeight;
    float renderOffset;
    ...
    public RdTextField(@Null String text, Skin skin) {
        this(text, skin.get(RdTextFieldStyle.class));
    }

    public RdTextField(@Null String text, Skin skin, String styleName) {
        this(text, skin.get(styleName, RdTextFieldStyle.class));
    }

    public RdTextField(@Null String text, RdTextFieldStyle style) {
        setStyle(style);
    ...
    public RdTextField(@Null String text) {
        this(text, RdAssetManager.current().getSkin());
    }

    public RdTextField(@Null String text, String styleName) {
        this(text, RdAssetManager.current().getSkin(), styleName);
    }

    /** returns true if the cursor is hovered */
    ...
    private final float cursorX = 0;

    protected int letterUnderCursor (float x) {
        x -= textOffset + fontOffset - cursorX - glyphPositions.get(visibleTextStart);
    ...
    public void setStyle (RdTextFieldStyle style) {
        ...
        if (style.messageFont != null && style.messageFontColor != null) {
            labelStyle = new RdLabel.RdLabelStyle(style.messageFont, style.messageFontColor);

        }
        messageLabel = new RdLabel(" ", labelStyle);

        // TODO
        //textHeight = style.font.getCapHeight() - style.font.getDescent() * 2;
        textHeight = label.getPrefHeight();
        if (minTextHeight == 0) minTextHeight = label.getPrefHeight();
    ...
    /** Returns the text field's style. Modifying the returned style may not have an effect until {@link #setStyle(RdTextFieldStyle)}
     * is called. */
    public RdTextFieldStyle getStyle () {
        return style;
    }
    ...
    protected void calculateOffsets () {
        ...
        if (hasSelection) {
            ...
            selectionX = minX;
            selectionWidth = maxX - minX - cursorX;
        }
    }
    ...
    protected @Null Drawable getBackgroundDrawable () {
        ...
        if (isOver) return style.overBackground;
    ...
    public void draw (Batch batch, float parentAlpha) {
        ...
        if (focused && hasSelection && selection != null) {
            ...
            drawSelection(selection, batch, font,
                    x + bgLeftWidth, y + textY - getLineHeight() * 1.5f);
        }
        ...
        if (displayText.length() == 0) {
            if ((!focused || disabled) && messageText != null) {
                ...
                // ???
                Font mFont = style.messageFont != null ? style.messageFont : font;

                if (style.messageFontColor != null) {
                    messageLabel.setColor(style.messageFontColor.r, style.messageFontColor.g, style.messageFontColor.b,
                            style.messageFontColor.a * color.a * parentAlpha);
                } else {
                    messageLabel.setColor(0.7f, 0.7f, 0.7f, color.a * parentAlpha);
                }

                // y + textY + yOffset
                drawMessageText(batch, mFont, x + bgLeftWidth, y + textY + yOffset / 2,
                        width - bgLeftWidth - bgRightWidth);
            }

        }

        label.setColor(fontColor.r, fontColor.g, fontColor.b, fontColor.a * color.a * parentAlpha);
        drawText(batch, font, x + bgLeftWidth, y + textY + yOffset / 2,
                width - bgLeftWidth - bgRightWidth);

        if (!disabled && cursorOn && cursorPatch != null) {
            // / 4
            //TextField
            drawCursor(cursorPatch, batch, font,
                    x + bgLeftWidth, y + textY - getLineHeight() * 1.5f);
        }
    }

    protected float getLineHeight() {
        return Math.max(minTextHeight, label.getWorkingLayout().getLine(0).height);
    }
    ...
    protected void drawSelection (Drawable selection, Batch batch, Font font, float x, float y) {
        // + font.descent
        // - textHeight
        selection.draw(batch, x + textOffset + selectionX + fontOffset + style.cursor.getMinWidth(),
                y, selectionWidth, textHeight);
    }

    protected void drawText (Batch batch, Font font, float x, float y, float maxWidth) {
        //layout.setPosition(x + textOffset, y, visibleTextStart);
        //float delta = cWidth * displayText.length();
        //displayText.
    ...
    protected void drawMessageText (Batch batch, Font font, float x, float y, float maxWidth) {
        // TODO
        ...
        messageLabel.setWidth(maxWidth);
        messageLabel.setText(messageText);

        messageLabel.act(Gdx.graphics.getDeltaTime());
        messageLabel.draw(batch, getColor().a);
    }
    ...
    protected void drawCursor (Drawable cursorPatch, Batch batch, Font font, float x, float y) {
        ...
        cursorPatch.draw(batch,
                x + textOffset + glyphPositions.get(cursor)
                        - glyphPositions.get(visibleTextStart) + fontOffset + cursorX,
                y, cursorPatch.getMinWidth(), textHeight);
    }

    void updateDisplayText () {
        Font font = style.font;
        label.getFont().defaultValue = label.getFont().mapping.get(' ');

        label.setText(text.replace('\r', ' ').replace('\n', ' '));

        if (passwordMode && font.mapping.containsKey(passwordCharacter)) {
            for (int ln = 0; ln < label.getWorkingLayout().lines(); ln++) {
                Line line = label.getWorkingLayout().getLine(ln);
        ...
        float end = 0f;
        if(label.getWorkingLayout().lines() != 0) {
            end = font.calculateXAdvances(label.getWorkingLayout().getLine(0), glyphPositions);
        } else
            fontOffset = 0;
    ...
    void paste (@Null String content, boolean fireChangeEvent) {
        ...
        IntMap<Font.GlyphRegion> mapping = label.getFont().mapping;
...
//		System.out.println("End of paste(): " + label.layout + "\n text: " + text);
    ...
    public void next (boolean up) {
        ...
        while (true) {
            RdTextField textField = current.findNextTextField(stage.getActors(), null, bestCoords, currentCoords, up);
    ...
    /** @return May be null. */
    private @Null RdTextField findNextTextField (Array<Actor> actors, @Null RdTextField best, Vector2 bestCoords,
                                                 Vector2 currentCoords, boolean up) {
        for (int i = 0, n = actors.size; i < n; i++) {
            Actor actor = actors.get(i);
            if (actor instanceof RdTextField) {
                if (actor == this) continue;
                RdTextField textField = (RdTextField) actor;
    ...
    public interface TextFieldListener {
        void keyTyped (RdTextField textField, char c);
    }

    /** Interface for filtering characters entered into the text field.
     * @author mzechner */
    public interface TextFieldFilter {
        boolean acceptChar (RdTextField textField, char c);

        class DigitsOnlyFilter implements TextFieldFilter {
            public boolean acceptChar (RdTextField textField, char c) {
    ...
    /** The default {@link OnscreenKeyboard} used by all {@link RdTextField} instances. Just uses
    ...
    public class TextFieldClickListener extends ClickListener {
        ...
        public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
            ...
            Stage stage = getStage();
            if (stage != null) stage.setKeyboardFocus(RdTextField.this);
            keyboard.show(true);
            hasSelection = true;
            return true;
        }
        ...
        /** Checks if focus traversal should be triggered. The default implementation uses {@link RdTextField#focusTraversal} and the
         * typed character, depending on the OS.
         * @param character The character that triggered a possible focus traversal.
         * @return true if the focus should change to the {@link com.badlogic.gdx.scenes.scene2d.ui.TextField#next(boolean) next} input field. */
        ...
        public boolean keyTyped (InputEvent event, char character) {
            if (disabled) return false;
            if ((character == CARRIAGE_RETURN || character == NEWLINE) && ignoreEnter)
                return false;
            ...
            if (checkFocusTraversal(character))
                ...
                boolean add = enter ? writeEnters : (!onlyFontChars || label.getFont().mapping.containsKey(character));
                ...
                if (add || remove) {
                    ...
                    if (add && !remove) {
                        ...
                        if (!enter && filter != null && !filter.acceptChar(RdTextField.this, character)) return true;
            ...
            if (listener != null) listener.keyTyped(RdTextField.this, character);
            return true;
        }
    }

    /** The style for a text field, see {@link RdTextField}.
     * @author mzechner
     * @author Nathan Sweet
     * @author Igor Ivanov
     * */
    static public class RdTextFieldStyle {

        public Font font;
        public Color fontColor;
        public @Null Color focusedFontColor, disabledFontColor;
        public @Null Drawable background, focusedBackground, overBackground,
                disabledBackground, cursor, selection;
        public @Null Font messageFont;
        public @Null Color messageFontColor;
        public float scaleText = 1;

        public RdTextFieldStyle() {}

        public RdTextFieldStyle(Font font, Color fontColor, @Null Drawable cursor, @Null Drawable selection,
        ...
        public RdTextFieldStyle(RdTextFieldStyle style) {
            font = style.font;
