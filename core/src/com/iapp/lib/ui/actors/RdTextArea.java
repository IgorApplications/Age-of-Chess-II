...
public class RdTextArea extends RdTextField {
    ...
    public float getCursorX () {
        ...
        if (!(cursor >= glyphPositions.size || cursorLine * 2 >= linesBreak.size)) {
            ...
            float res = 0;
            if (lineStart < glyphPositions.size) res = glyphPositions.get(lineStart);
            textOffset = glyphPositions.get(cursor) - res + glyphOffset;
        }
    ...
    int delete (boolean fireChangeEvent) {
        return super.delete(fireChangeEvent);
    }
    ...
    protected void drawSelection (Drawable selection, Batch batch, Font font, float x, float y) {
        ...
        while (i + 1 < linesBreak.size && i < linesShowing * 2) {
            ...
            if (!((minIndex < lineStart && minIndex < lineEnd && maxIndex < lineStart && maxIndex < lineEnd)
                ...
                float selectionX = 0;
                if (start < glyphPositions.size)
                    selectionX = glyphPositions.get(start) - glyphPositions.get(lineStart);
                float selectionWidth = 0;
                if (end < glyphPositions.size)
                    selectionWidth = glyphPositions.get(end) - glyphPositions.get(start);
