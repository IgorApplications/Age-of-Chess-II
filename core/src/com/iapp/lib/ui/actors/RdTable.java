 ...
 * <p>
 * Children added with {@link #add(Actor...)} (and similar methods returning a {@link RdCell}) are laid out in rows and columns.
...
public class RdTable extends WidgetGroup {
    ...
    private final Array<RdCell> cells = new Array<>(4);
    private final RdCell rdCellDefaults;
    private final Array<RdCell> columnDefaults = new Array<>(2);
    private RdCell rowDefaults;
    ...
    RdTable.Debug debug = RdTable.Debug.none;
    Array<RdTable.DebugRect> debugRects;
    ...
    private RdCell obtainCell () {
        RdCell rdCell = cellPool.obtain();
        rdCell.setTable(this);
        return rdCell;
    }
    ...
    /** @see #setBackground(Drawable) */
    public RdTable background (@Null Drawable background) {
        setBackground(background);
        return this;
    }

    /** @see #setBackground(String) */
    public RdTable background (String drawableName) {
    ...
    public <T extends Actor> RdCell<T> add(@Null T actor) {
        ...
        if (cellCount > 0) {
            ...
            RdCell lastRdCell = cells.peek();
            if (!lastRdCell.endRow) {
                rdCell.column = lastRdCell.column + lastRdCell.colspan;
                rdCell.row = lastRdCell.row;
            } else {
                rdCell.column = 0;
                rdCell.row = lastRdCell.row + 1;
            }
            ...
            if (rdCell.row > 0) {
                ...
                for (int i = cellCount - 1; i >= 0; i--) {
                    RdCell other = (RdCell)cells[i];
                    for (int column = other.column, nn = column + other.colspan; column < nn; column++) {
                        if (column == rdCell.column) {
                            rdCell.cellAboveIndex = i;
                            break outer;
                        }
                    }
                }
        ...
        rdCell.set(rdCellDefaults);
        if (rdCell.column < columnDefaults.size) rdCell.merge(columnDefaults.get(rdCell.column));
        rdCell.merge(rowDefaults);
    ...
    public void reset() {
        ...
        align = Align.center;
        debug(RdTable.Debug.none);
        rdCellDefaults.reset();
        for (int i = 0, n = columnDefaults.size; i < n; i++) {
            RdCell columnRdCell = columnDefaults.get(i);
            if (columnRdCell != null) cellPool.free(columnRdCell);
        }
    ...
    private void endRow() {
        ...
        for (int i = this.cells.size - 1; i >= 0; i--) {
            RdCell rdCell = (RdCell)cells[i];
            if (rdCell.endRow) break;
            rowColumns += rdCell.colspan;
        }
    ...
    public RdCell columnDefaults(int column) {
        ...
        if (rdCell == null) {
            ...
            if (column >= columnDefaults.size) {
                ...
                columnDefaults.add(rdCell);
            } else
                columnDefaults.set(column, rdCell);
        }
        return rdCell;
    }
    ...
    private void computeSize () {
        ...
        if (cellCount > 0 && !((RdCell)cells[cellCount - 1]).endRow) {
            endRow();
            implicitEndRow = true;
        }
        ...
        for (int i = 0; i < cellCount; i++) {
            ...
            if (c.cellAboveIndex != -1) {
                RdCell above = (RdCell)cells[c.cellAboveIndex];
    ...
    public void layout () {
        ...
        if (totalGrowWidth == 0)
            ...
            columnWeightedWidth = RdTable.columnWeightedWidth = ensureSize(RdTable.columnWeightedWidth, columns);
        ...
        if (totalGrowHeight == 0)
            ...
            rowWeightedHeight = RdTable.rowWeightedHeight = ensureSize(RdTable.rowWeightedHeight, rows);
        ...
        if (debug != RdTable.Debug.none) addDebugRects(x, y, tableWidth - hpadding, tableHeight - vpadding);
    }

    private void addDebugRects (float currentX, float currentY, float width, float height) {
        clearDebugRects();
        if (debug == RdTable.Debug.table || debug == RdTable.Debug.all) {
            // Table actor bounds.
        ...
        for (int i = 0, n = cells.size; i < n; i++) {
            RdCell c = cells.get(i);

            // Cell actor bounds.
            if (debug == RdTable.Debug.actor || debug == RdTable.Debug.all)
            ...
            spannedCellWidth -= c.computedPadLeft + c.computedPadRight;
            currentX += c.computedPadLeft;
            if (debug == RdTable.Debug.cell || debug == RdTable.Debug.all) {
    ...
    private void clearDebugRects () {
        ...
        RdTable.DebugRect.pool.freeAll(debugRects);
        debugRects.clear();
    }

    private void addDebugRect (float x, float y, float w, float h, Color color) {
        RdTable.DebugRect rect = RdTable.DebugRect.pool.obtain();
        rect.color = color;
        rect.set(x, y, w, h);
        debugRects.add(rect);
    }
    ...
    private void drawDebugRects (ShapeRenderer shapes) {
        ...
        for (int i = 0, n = debugRects.size; i < n; i++) {
            RdTable.DebugRect debugRect = debugRects.get(i);
    ...
    static public class DebugRect extends Rectangle {
        static Pool<RdTable.DebugRect> pool = Pools.get(RdTable.DebugRect.class);
        Color color;
    }
    ...
    static public Value backgroundTop = new Value() {
        public float get (@Null Actor context) {
            Drawable background = ((RdTable)context).background;
